...
import {EditEntry, Entry, EntryTransMiddleware, RawEntry} from "../types";
// @ts-ignore
import * as R from "ramda";
const r = R;

const bundleValueSetterPairs: EntryTransMiddleware = (entries: Entry[]): Entry[] => {
  // Utility function.
  ...
  const valuePairedEntries: {
    ...
    r.map(
      ({index, entry}: {index: number; entry: RawEntry}) => {
        ...
        const theValueName = eventName(entry.name);
